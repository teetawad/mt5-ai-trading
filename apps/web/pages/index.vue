<template>
  <div class="space-y-6">
    ...
    <UiCard
      title="Hourly Trading (Phase 27 — primary strategy)"
      :subtitle="`Session: ${dashboard?.hourly.sessionStatus ?? 'UNKNOWN'} / Mode: ${dashboard?.hourly.modeEnabled ? 'ENABLED' : 'DISABLED'} / Next candle analysis: ${dateTime(dashboard?.hourly.nextAnalysisAt)}`"
    >
      <template #actions>
        <NuxtLink
          to="/hourly"
          class="rounded-lg border border-sky-400/50 bg-sky-400/10 px-3 py-1.5 text-xs font-semibold text-sky-100 transition hover:bg-sky-400/20"
        >
          Open Hourly page
        </NuxtLink>
      </template>
      <div class="grid gap-4 lg:grid-cols-2">
        <div>
          <div class="text-xs font-semibold uppercase tracking-wide text-slate-500">Watchlist</div>
          <div class="mt-2 flex flex-wrap gap-2">
            <span
              v-for="entry in dashboard?.hourly.watchlist ?? []"
              :key="entry.id"
              class="rounded-full border px-2.5 py-1 text-[11px] font-semibold"
              :class="entry.enabled ? 'border-sky-500/40 bg-sky-500/10 text-sky-200' : 'border-slate-600 bg-slate-800 text-slate-400'"
            >
              {{ entry.symbol }}{{ entry.lastProcessedCandle ? ` · ${dateTime(entry.lastProcessedCandle)}` : '' }}
            </span>
            <span
              v-if="!(dashboard?.hourly.watchlist?.length)"
              class="text-xs text-slate-500"
            >No watchlist symbols configured.</span>
          </div>
        </div>
        <div>
          <div class="text-xs font-semibold uppercase tracking-wide text-slate-500">Daily loss used</div>
          <p class="mt-2 text-sm font-semibold text-slate-100">{{ money(dashboard?.hourly.dailyLossUsed) }}</p>
          <div class="mt-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Recent hourly signals</div>
          <ul class="mt-2 space-y-1 text-sm text-slate-300">
            <li
              v-for="signal in dashboard?.hourly.recentSignals ?? []"
              :key="signal.id"
            >
              {{ signal.symbol }} — {{ signal.side }} <span class="text-slate-500">({{ dateTime(signal.createdAt) }})</span>
            </li>
            <li
              v-if="!(dashboard?.hourly.recentSignals?.length)"
              class="text-slate-500"
            >
              No hourly signals yet.
            </li>
          </ul>
        </div>
      </div>
    ...
    >
      ...
      >
        ...
        >
          ...
          <td class="px-4 py-3 tabular-nums">{{ money(bracketSnapshot(proposal)?.entry ?? proposal.referencePrice) }}</td>
          <td class="px-4 py-3 tabular-nums text-rose-200">{{ money(bracketSnapshot(proposal)?.stopLoss) }}</td>
          <td class="px-4 py-3 tabular-nums text-emerald-200">{{ money(bracketSnapshot(proposal)?.takeProfit) }}</td>
...
type Proposal = {
  ...
  riskSnapshot: {
    result?: string;
    phase22?: BracketSnapshot;
    phase25?: BracketSnapshot;
    phase26?: BracketSnapshot;
    phase27?: BracketSnapshot;
  } | null;
};
...
type Dashboard = {
  ...
  hourly: {
    ...
    watchlist: HourlyWatchlistEntry[];
    recentSignals: HourlySignal[];
...
type HourlyWatchlistEntry = { id: string; symbol: string; enabled: boolean; lastProcessedCandle: string | null };
type HourlySignal = { id: string; symbol: string; side: string; createdAt: string };
type Snapshot = {
  id: string;
  cashBalance: string;
  portfolioEquity: string;
...
// Whichever phase-specific bracket the proposal actually carries — mirrors
// the server-side bracketFromProposal priority (phase27 ?? phase25 ??
// phase26 ?? phase22), since only one of these is ever set per proposal.
function bracketSnapshot(proposal: Proposal): BracketSnapshot | undefined {
  const snapshot = proposal.riskSnapshot;
  return snapshot?.phase27 ?? snapshot?.phase25 ?? snapshot?.phase26 ?? snapshot?.phase22;
}
