...
from strategy.intraday.analysis import IntradayAnalysis, analyze_multi_timeframe
...
class IntradaySessionConfigRequest(BaseModel):
...
class IntradayConfigRequest(BaseModel):
    ...
    session: IntradaySessionConfigRequest = IntradaySessionConfigRequest()

    model_config = {"arbitrary_types_allowed": True}

    def to_config(self) -> IntradayStrategyConfig:
...
class IntradayAnalyzeRequest(BaseModel):
    symbol: str
    config: IntradayConfigRequest = IntradayConfigRequest()
...
@router.post("/analyze", response_model=IntradayAnalysis)
async def analyze(
